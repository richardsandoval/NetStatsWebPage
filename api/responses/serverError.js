...
module.exports = function serverError(data, options) {
    ...
    if (req.wantsJSON) {
        ...
        if (data.invalidAttributes) {
            var queue = '';
            for (var key in data.invalidAttributes) {
                queue += "Error in " + key + ' field with: ';
                data.invalidAttributes[key].forEach(function (element) {
                    console.log(element);
                    queue += element.rule + ', ';
                });
            ...
            data.raw = queue.substring(0, queue.length - 1);
        }
        return res.jsonx({
            error: {
                code: data.error ? data.error : "401",
                message: data.raw ? data.raw : "Error in server",
                status: false
            },
            data: {}
        });
